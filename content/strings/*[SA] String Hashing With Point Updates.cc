/**
 * Author: Sarwar Alam
 * Description: string hashing with point updates
 */

struct Node {
	int64_t fwd, rev;
	int len;
	Node(int64_t f, int64_t r, int l) { fwd = f, rev = r, len = l; }
	Node() { fwd = rev = len = 0; }
};
...
Node query(int node, int tL, int tR, int qL, int qR) {
	if (tL >= qL && tR <= qR) return Node(st[node].fwd, st[node].rev, st[node].len);
	if (tR < qL || tL > qR) return Node(0, 0, 0);
	int mid = (tL + tR) / 2;
