...
private:
	...
	T query(int ss, int se, int si, int qs, int qe) {
		...
		if (se < qs || qe < ss) return neutral;
		if (qs <= ss && qe >= se) return segt[si];
...
int lazy_to_seg(int seg, int lazy_v, int l, int r) { return seg + (lazy_v * (r - l + 1)); }
int lazy_to_lazy(int curr_lazy, int input_lazy) { return curr_lazy + input_lazy; }